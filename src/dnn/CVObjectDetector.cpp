...
CVObjectDetector::CVObjectDetector(std::string weights, std::string config, std::string framework) : BaseObjectDetector() {
  _model = std::shared_ptr<cv::dnn::Net>(new cv::dnn::Net(cv::dnn::readNet(weights, config, framework)));
