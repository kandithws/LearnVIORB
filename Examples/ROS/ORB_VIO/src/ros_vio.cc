...
int main(int argc, char **argv) {
     ...
     */
    std::string vocab_path(argv[1]), config_path(argv[2]), bagfile_path;
    ...
    if (realtime_mode) {
        imagesub = nh.subscribe(ORB_SLAM2::Config::getInstance().RuntimeParams().image_topic, /*200*/ 2, &ORBVIO::MsgSynchronizer::imageCallback, &msgsync);
    ...
    ros::Rate r(1000);
    double test_discard_time = ORB_SLAM2::Config::getInstance().RuntimeParams().discard_time;
    if (!realtime_mode) {
        ...
        BOOST_FOREACH(rosbag::MessageInstance
                        ...
                        if (bdata) {
                            ...
                            cv::Mat im = cv_ptr->image.clone();
                            {
                                ...
                                if (imageMsg->header.stamp.toSec() < startT + test_discard_time)
